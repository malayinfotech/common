...
func VerifyUplinkOrderSignature(ctx context.Context, publicKey storx.PiecePublicKey, signed *pb.Order) (err error) {
	ctx = rpctracing.WithoutDistributedTracing(ctx)
	defer mon.Task()(&ctx)(&err)
...
func VerifyUplinkPieceHashSignature(ctx context.Context, publicKey storx.PiecePublicKey, signed *pb.PieceHash) (err error) {
	ctx = rpctracing.WithoutDistributedTracing(ctx)
	defer mon.Task()(&ctx)(&err)
