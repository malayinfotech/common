...
// provided store and bucket.
func EncryptPath(bucket string, path paths.Unencrypted, pathCipher storx.CipherSuite, store *Store) (
	encPath paths.Encrypted, err error) {
...
func EncryptIterator(iter paths.Iterator, cipher storx.CipherSuite, key *storx.Key) (string, error) {
	var pb pathBuilder
	for !iter.Done() {
		component := iter.Next()
...
func EncryptPathRaw(raw string, cipher storx.CipherSuite, key *storx.Key) (string, error) {
...
// provided store and bucket.
func DecryptPath(bucket string, path paths.Encrypted, pathCipher storx.CipherSuite, store *Store) (
	encPath paths.Unencrypted, err error) {
...
func DecryptIterator(iter paths.Iterator, cipher storx.CipherSuite, key *storx.Key) (string, error) {
	var pb pathBuilder
	for !iter.Done() {
		component := iter.Next()
...
// preferred if possible.
func DecryptPathRaw(raw string, cipher storx.CipherSuite, key *storx.Key) (string, error) {
...
func DeriveContentKey(bucket string, path paths.Unencrypted, store *Store) (key *storx.Key, err error) {
...
func DerivePathKey(bucket string, path paths.Unencrypted, store *Store) (key *storx.Key, err error) {
	_, remaining, base := store.LookupUnencrypted(bucket, path)
	if base == nil {
...
func derivePathKeyComponent(key *storx.Key, component string) (*storx.Key, error) {
...
func encryptPathComponent(comp string, cipher storx.CipherSuite, key *storx.Key) (string, error) {
	if cipher == storx.EncNull {
		return comp, nil
	}

	if cipher == storx.EncNullBase64URL {
	...
	nonceSize := storx.NonceSize
	if cipher == storx.EncAESGCM {
...
func decryptPathComponent(comp string, cipher storx.CipherSuite, key *storx.Key) (string, error) {
	if comp == "" {
		return "", nil
	}
