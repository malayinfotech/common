...
func NewAESGCMEncrypter(key *storx.Key, startingNonce *AESGCMNonce, encryptedBlockSize int) (Transformer, error) {
...
// startingNonce.
func NewAESGCMDecrypter(key *storx.Key, startingNonce *AESGCMNonce, encryptedBlockSize int) (Transformer, error) {
	block, err := aes.NewCipher(key[:])
	if err != nil {
		return nil, Error.Wrap(err)
	}
...
func EncryptAESGCM(data []byte, key *storx.Key, nonce *AESGCMNonce) (cipherData []byte, err error) {
	block, err := aes.NewCipher(key[:])
	if err != nil {
...
func DecryptAESGCM(cipherData []byte, key *storx.Key, nonce *AESGCMNonce) (data []byte, err error) {
	if len(cipherData) == 0 {
