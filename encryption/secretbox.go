...
func NewSecretboxEncrypter(key *storx.Key, startingNonce *storx.Nonce, encryptedBlockSize int) (Transformer, error) {
...
func calcNonce(startingNonce *storx.Nonce, blockNum int64) (rv *storx.Nonce, err error) {
	rv = new(storx.Nonce)
...
// startingNonce.
func NewSecretboxDecrypter(key *storx.Key, startingNonce *storx.Nonce, encryptedBlockSize int) (Transformer, error) {
	if encryptedBlockSize <= secretbox.Overhead {
		return nil, ErrInvalidConfig.New("encrypted block size %d too small", encryptedBlockSize)
	}
...
func EncryptSecretBox(data []byte, key *storx.Key, nonce *storx.Nonce) (cipherData []byte, err error) {
...
func DecryptSecretBox(cipherData []byte, key *storx.Key, nonce *storx.Nonce) (data []byte, err error) {
