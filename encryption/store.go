...
type Store struct {
	...
	defaultKey        *storx.Key
	defaultPathCipher storx.CipherSuite
	...
	// will return storx.EncNullBase64URL. Iterate is unaffected.
...
func (s *Store) GetDefaultPathCipher() storx.CipherSuite {
...
func (s *Store) Add(bucket string, unenc paths.Unencrypted, enc paths.Encrypted, key storx.Key) error {
...
func (s *Store) AddWithCipher(bucket string, unenc paths.Unencrypted, enc paths.Encrypted, key storx.Key, pathCipher storx.CipherSuite) error {
...
func (s *Store) LookupUnencrypted(bucket string, path paths.Unencrypted) (
	...
	defer func() {
		if base != nil && s.EncryptionBypass {
			base.PathCipher = storx.EncNullBase64URL
		}
	}()
...
func (s *Store) Iterate(fn func(string, paths.Unencrypted, paths.Encrypted, storx.Key) error) error {
...
func (n *node) iterate(fn func(string, paths.Unencrypted, paths.Encrypted, storx.Key) error, bucket string) error {
	if n.base != nil {
...
func (s *Store) IterateWithCipher(fn func(string, paths.Unencrypted, paths.Encrypted, storx.Key, storx.CipherSuite) error) error {
	for bucket, root := range s.roots {
...
func (n *node) iterateWithCipher(fn func(string, paths.Unencrypted, paths.Encrypted, storx.Key, storx.CipherSuite) error, bucket string) error {
	if n.base != nil {
