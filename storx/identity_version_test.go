// Copyright (C) 2019 Storx Labs, Inc.
// See LICENSE for copying information.

package storx_test

import (
	"crypto/x509"
	"testing"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"

	"common/identity"
	"common/peertls"
	"common/peertls/extensions"
	"common/peertls/testpeertls"
	"common/peertls/tlsopts"
	"common/storx"
)

func TestLatestVersion(t *testing.T) {
	version := storx.LatestIDVersion()
	assert.Equal(t, storx.V0, version.Number)
}

func TestIDVersionFromCert(t *testing.T) {
	for versionNumber := range storx.IDVersions {
		t.Logf("id version %d", versionNumber)
		_, chain, err := testpeertls.NewCertChain(2, versionNumber)
		require.NoError(t, err)

		cert := chain[peertls.CAIndex]

		version, err := storx.IDVersionFromCert(cert)
		require.NoError(t, err)

		assert.Equal(t, versionNumber, version.Number)
	}
}

func TestIDVersionInVersions_match(t *testing.T) {
	testcases := []struct {
		name          string
		versionNumber storx.IDVersionNumber
		versionsStr   string
	}{
		{"single version", storx.V0, "0"},
		{"beginning of version range", storx.V0, "0-1"},
		{"latest alias", storx.LatestIDVersion().Number, "latest"},
	}

	for _, testcase := range testcases {
		t.Log(testcase.name)
		err := storx.IDVersionInVersions(testcase.versionNumber, testcase.versionsStr)
		assert.NoError(t, err)
	}
}

func TestIDVersionInVersions_error(t *testing.T) {
	testcases := []struct {
		name          string
		versionNumber storx.IDVersionNumber
		versionsStr   string
	}{
		{"version range", storx.V0, "1-2"},
		{"malformed PeerIDVersions", storx.V0, "0-"},
	}

	for _, testcase := range testcases {
		t.Log(testcase.name)
		err := storx.IDVersionInVersions(testcase.versionNumber, testcase.versionsStr)
		assert.Error(t, err)
	}
}

func TestIDVersionExtensionHandler_success(t *testing.T) {
	_, identityV1Chain, err := testpeertls.NewCertChain(2, storx.V0)
	assert.NoError(t, err)

	latestVersionChain := identityV1Chain

	testcases := []struct {
		name     string
		versions string
		chain    []*x509.Certificate
	}{
		{"V0", "0", identityV1Chain},
		{"any with V0 chain", "*", identityV1Chain},
		{"latest version", "latest", latestVersionChain},
	}

	for _, testcase := range testcases {
		t.Log(testcase.name)
		opts := &extensions.Options{PeerIDVersions: testcase.versions}

		cert := testcase.chain[peertls.CAIndex]
		ext := cert.Extensions[len(cert.Extensions)-1]
		err := storx.IDVersionHandler.NewHandlerFunc(opts)(ext, identity.ToChains(testcase.chain))
		assert.NoError(t, err)

		extensionMap := tlsopts.NewExtensionsMap(testcase.chain...)
		handlerFuncMap := extensions.DefaultHandlers.WithOptions(opts)

		err = extensionMap.HandleExtensions(handlerFuncMap, identity.ToChains(testcase.chain))
		assert.NoError(t, err)
	}
}

func TestV0LegacyCertCompatibility(t *testing.T) {
	unsigned, err := identity.FullIdentityFromPEM(
		[]byte("-----BEGIN CERTIFICATE-----\\ne9ydnkqoISRCZuqIvWDGPkxFjdEp\n-----END CERTIFICATE-----\n-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"),
		[]byte("-----BEGIN EC PRIVATE KEY-----\nMHcCAQEEICvE+Bd39LJ3VVf/SBdkw/IPjyVmMWq8Sr7GuWzkfdpJoAoGCCqGSM49\nAwEHoUQDQgAEVoir4M8l+8y/xHAVcikYbEs4cJuy8byIzIBqu/TiXxCm08+dIQqK\nh34oZuFDqkQlnR5YTINuQD1GxQb9rE1f3g==\n-----END EC PRIVATE KEY-----\n"),
	)
	require.NoError(t, err)
	require.Equal(t, unsigned.ID, storx.NodeID{80, 20, 7, 41, 199, 54, 178, 46, 100, 217, 12, 23, 0, 168, 87, 93, 97, 118, 238, 48, 128, 209, 31, 245, 164, 235, 108, 191, 171, 206, 224, 0})

	signed, err := identity.FullIdentityFromPEM(
		[]byte("-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n-----BEGIN CERTIFICATE-----\\nOOfce7bVuJlxvsnNfkkz\n-----END CERTIFICATE-----\n-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"),
		[]byte("-----BEGIN PRIVATE KEY-----\nMIGHAgEAMBMGByqGSM49AgEGCCqGSM49AwEHBG0wawIBAQQgzsFsVqt/GdqQlIIJ\nHH2VQNndv1A1fTk/35VPNzLW04ehRANCAATzXrIfcBZAHHxPdFD2PFRViRwe6eWf\nQipaF4iXQmHAW79X4mDx0BibjFfvmzurnYSlyIMZn3jp9RzbLMfnA10C\n-----END PRIVATE KEY-----\n"),
	)
	require.NoError(t, err)
	require.Equal(t, signed.ID, storx.NodeID{14, 210, 138, 187, 40, 19, 225, 132, 161, 233, 139, 15, 102, 5, 196, 145, 30, 164, 104, 199, 232, 67, 62, 181, 131, 224, 252, 167, 206, 172, 48, 0})
}
