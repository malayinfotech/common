...
func NodeIDFromCertPath(certPath string) (storx.NodeID, error) {
	/* #nosec G304 */ // Subsequent calls ensure that the file is a certificate
	certBytes, err := os.ReadFile(certPath)
	if err != nil {
		return storx.NodeID{}, err
	}
	return NodeIDFromPEM(certBytes)
}

// NodeIDFromPEM loads a node ID from certificate bytes.
func NodeIDFromPEM(pemBytes []byte) (storx.NodeID, error) {
	chain, err := pkcrypto.CertsFromPEM(pemBytes)
	if err != nil {
		return storx.NodeID{}, Error.New("invalid identity certificate")
	}
	if len(chain)-1 < peertls.CAIndex {
		return storx.NodeID{}, Error.New("no CA in identity certificate")
	}
...
func NodeIDFromCert(cert *x509.Certificate) (id storx.NodeID, err error) {
...
func NodeIDFromKey(k crypto.PublicKey, version storx.IDVersion) (storx.NodeID, error) {
	idBytes, err := peertls.DoubleSHA256PublicKey(k)
	if err != nil {
		return storx.NodeID{}, storx.ErrNodeID.Wrap(err)
	}
	return storx.NewVersionedID(idBytes, version), nil
}
...
func (fi *FullIdentity) Version() (storx.IDVersion, error) {
	return storx.IDVersionFromCert(fi.CA)
}
