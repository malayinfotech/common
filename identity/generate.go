...
func GenerateKey(ctx context.Context, minDifficulty uint16, version storx.IDVersion) (
	k crypto.PrivateKey, id storx.NodeID, err error) {
	...
	return k, id, storx.ErrNodeID.Wrap(err)
}
...
type GenerateCallback func(crypto.PrivateKey, storx.NodeID) (done bool, err error)

// GenerateKeys continues to generate keys until found returns done == false,
// or the ctx is canceled.
func GenerateKeys(ctx context.Context, minDifficulty uint16, concurrency int, version storx.IDVersion, found GenerateCallback) (err error) {
	defer mon.Task()(&ctx)(&err)
