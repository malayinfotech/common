...
func NewCA(ctx context.Context, opts NewCAOptions) (_ *FullCertificateAuthority, err error) {
	...
	version, err := storx.GetIDVersion(opts.VersionNumber)
	if err != nil {
		return nil, err
	}
	...
	err = GenerateKeys(ctx, minimumLoggableDifficulty, int(opts.Concurrency), version,
		func(k crypto.PrivateKey, id storx.NodeID) (done bool, err error) {
			if opts.Logger != nil {
	...
	if err := extensions.AddExtraExtension(ct, storx.NewVersionExt(version)); err != nil {
		return nil, err
	}
...
func (caS CASetupConfig) Create(ctx context.Context, logger io.Writer) (*FullCertificateAuthority, error) {
	...
	version, err := storx.GetIDVersion(storx.IDVersionNumber(caS.VersionNumber))
	if err != nil {
		return nil, err
	}
...
func (ca *FullCertificateAuthority) Version() (storx.IDVersion, error) {
	return storx.IDVersionFromCert(ca.Cert)
}
