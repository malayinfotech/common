...
// and a full identity.
type IdentityTest func(*testing.T, storx.IDVersion, *identity.FullIdentity)

// SignerTest is a function that takes a testing struct, an identity version
// and a full certificate authority.
type SignerTest func(*testing.T, storx.IDVersion, *identity.FullCertificateAuthority)
...
func NewTestCA(ctx context.Context) (*identity.FullCertificateAuthority, error) {
	return identity.NewCA(ctx, identity.NewCAOptions{
		VersionNumber: storx.LatestIDVersion().Number,
		Difficulty:    9,
		Concurrency:   4,
	})
}
...
func IdentityVersionsTest(t *testing.T, test IdentityTest) {
	for vn, v := range storx.IDVersions {
		versionNumber := vn
		version := v
