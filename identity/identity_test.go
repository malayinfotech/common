	...
	testidentity.CompleteIdentityVersionsTest(t, func(t *testing.T, version storx.IDVersion, ident *identity.FullIdentity) {
		...
		{ // pre-save version assertions
			...
			if ident.ID.Version().Number == 0 {
				...
				assert.Equal(t, ident.ID.Version().Number, storx.IDVersionNumber(versionExt.Value[0]))
			} else {
...
func TestConfig_Save(t *testing.T) {
	...
	testidentity.IdentityVersionsTest(t, func(t *testing.T, version storx.IDVersion, ident *identity.FullIdentity) {
		identCfg := &identity.Config{
			CertPath: ctx.File("chain.pem"),
			KeyPath:  ctx.File("key.pem"),
		}
...
func TestVersionedNodeIDFromKey(t *testing.T) {
	...
	for _, v := range storx.IDVersions {
		version := v
