...
func GenerateServerConf(proto pb.NoiseProtocol, ident *identity.FullIdentity) (noise.Config, error) {
	...
	// https://github.com/storx/storx/blob/b022f371d24b64d9435dc02f5cc0de8bf6bff718/docs/blueprints/noise.md
	...
	entropy, err := identityBasedEntropy("storx noise server key", ident)
	if err != nil {
		return noise.Config{}, err
	}
...
func ValidateKeyAttestation(ctx context.Context, attestation *pb.NoiseKeyAttestation, expectedNodeID storx.NodeID) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func ValidateSessionAttestation(ctx context.Context, attestation *pb.NoiseSessionAttestation, expectedNodeID storx.NodeID) (err error) {
	defer mon.Task()(&ctx)(&err)
	peer, err := identity.DecodePeerIdentity(ctx, attestation.NodeCertchain)
	if err != nil {
		return Error.Wrap(err)
