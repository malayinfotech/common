...
func (d Dialer) DialNode(ctx context.Context, nodeURL storx.NodeURL, opts DialOptions) (_ *Conn, err error) {
	defer mon.Task()(&ctx, "node: "+nodeURL.String())(&err)

	if opts.ReplaySafe && nodeURL.NoiseInfo != (storx.NoiseInfo{}) {
		vals := url.Values{}
...
func (d Dialer) DialNodeURL(ctx context.Context, nodeURL storx.NodeURL) (_ *Conn, err error) {
...
func (d Dialer) DialAddressInsecure(ctx context.Context, address string) (_ *Conn, err error) {
	...
	return d.dialPool(ctx, "insecure:"+address, func(ctx context.Context) (drpc.Conn, *tls.ConnectionState, error) {
		// check for a quic rollout, and if not, force tcp.
		ctx = setQUICRollout(ctx, storx.NodeURL{})
...
func (d Dialer) dialNoiseConn(ctx context.Context, address string, noiseInfo storx.NoiseInfo) (_ drpc.Conn, _ *tls.ConnectionState, err error) {
	defer mon.Task()(&ctx)(&err)
