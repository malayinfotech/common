...
func NewEncryptionAccessWithDefaultKey(defaultKey *storx.Key) *EncryptionAccess {
...
// Use (*Project).SaltedKeyFromPassphrase to generate a default key.
func (s *EncryptionAccess) SetDefaultKey(defaultKey *storx.Key) {
	s.Store.SetDefaultKey(defaultKey)
}

// SetDefaultPathCipher sets which cipher suite to use by default.
func (s *EncryptionAccess) SetDefaultPathCipher(defaultPathCipher storx.CipherSuite) {
...
func (s *EncryptionAccess) toProto() (*pb.EncryptionAccess, error) {
	...
	err := s.Store.IterateWithCipher(func(bucket string, unenc paths.Unencrypted, enc paths.Encrypted, key storx.Key, pathCipher storx.CipherSuite) error {
...
func parseEncryptionAccessFromProto(p *pb.EncryptionAccess) (*EncryptionAccess, error) {
	...
	if len(p.DefaultKey) > 0 {
		if len(p.DefaultKey) != len(storx.Key{}) {
			return nil, errors.New("invalid default key in encryption access")
		}
		var defaultKey storx.Key
		copy(defaultKey[:], p.DefaultKey)
		access.SetDefaultKey(&defaultKey)
	}

	access.SetDefaultPathCipher(storx.CipherSuite(p.DefaultPathCipher))
	...
	if p.DefaultPathCipher == pb.CipherSuite_ENC_UNSPECIFIED {
		access.SetDefaultPathCipher(storx.EncAESGCM)
	}

	for _, entry := range p.StoreEntries {
		if len(entry.Key) != len(storx.Key{}) {
		...
		err := access.Store.AddWithCipher(
			...
			key,
			storx.CipherSuite(entry.PathCipher),
		)
		if err != nil {
