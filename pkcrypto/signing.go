...
const (
	// StorxPSSSaltLength holds the correct value for the PSS salt length
	// when signing with RSA in Storx code and verifying RSA signatures
	// from Storx.
	StorxPSSSaltLength = rsa.PSSSaltLengthAuto

	// StorxRSAKeyBits holds the number of bits to use for new RSA keys
	// by default.
	StorxRSAKeyBits = 2048
)
...
func GeneratePrivateKey() (crypto.PrivateKey, error) {
	...
	// return GeneratePrivateRSAKey(StorxRSAKeyBits)
}
