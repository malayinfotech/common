...
var (
	mainpkg = flag.String("pkg", "common/pb", "main package name")
...
// storx/drpc/cmd/protoc-gen-go-drpc
...
func main() {
...

	{
		...
		overrideImports := ",Mgoogle/protobuf/timestamp.proto=storx/common/pb"
		args := []string{
			"--lint_out=.",
...

	{
		...
		out, err := exec.Command("goimports", "-local", "storx", "-w", ".").CombinedOutput()
		if len(out) > 0 {
			fmt.Println(string(out))
		}
		check(err)
