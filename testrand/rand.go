...
func PieceID() storx.PieceID {
	var id storx.PieceID
	Read(id[:])
	return id
}
...
func SerialNumber() storx.SerialNumber {
	var serial storx.SerialNumber
	Read(serial[:])
	return serial
}

// StreamID creates a random stream ID.
func StreamID(size int) storx.StreamID {
	return storx.StreamID(BytesInt(size))
}

// SegmentID creates a random segment ID.
func SegmentID(size int) storx.SegmentID {
	return storx.SegmentID(BytesInt(size))
}
...
//
// NOTE: This may not generate values that cover all valid values (for Storx or
...
// Path creates a random path that Storx should support.
func Path() string {
	n := rand.Intn(1024) + 1
