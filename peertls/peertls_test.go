...
func TestVerifyPeerFunc(t *testing.T) {
	_, chain, err := testpeertls.NewCertChain(2, storx.LatestIDVersion().Number)
	require.NoError(t, err)
...
func TestVerifyPeerCertChains(t *testing.T) {
	...
	keys, chain, err := testpeertls.NewCertChain(2, storx.LatestIDVersion().Number)
	require.NoError(t, err)
...
func TestVerifyCAWhitelist(t *testing.T) {
	_, chain2, err := testpeertls.NewCertChain(2, storx.LatestIDVersion().Number)
	require.NoError(t, err)
	...
	_, unrelatedChain, err := testpeertls.NewCertChain(1, storx.LatestIDVersion().Number)
	require.NoError(t, err)
	...
	_, chain3, err := testpeertls.NewCertChain(3, storx.LatestIDVersion().Number)
	require.NoError(t, err)
...
func TestAddExtraExtension(t *testing.T) {
	_, chain, err := testpeertls.NewCertChain(1, storx.LatestIDVersion().Number)
	require.NoError(t, err)
