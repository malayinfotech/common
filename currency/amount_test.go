...
func TestAmountFromBigFloatAndAmountAsBigFloat(t *testing.T) {
	...
	tests := []struct {
		...
		{"zero", big.NewFloat(0), StorxToken, 0, false},
		{"one", big.NewFloat(1), USDollars, 100, false},
		{"negative", big.NewFloat(-1), Bitcoin, -100000000, false},
		{"smallest", big.NewFloat(1e-8), StorxToken, 1, false},
		{"minus smallest", big.NewFloat(-1e-8), StorxToken, -1, false},
		...
		{"infinity", parseFloat("Inf"), StorxToken, 0, true},
	}
...
func TestAmountJSONMarshal(t *testing.T) {
	tests := []struct {
		...
		{
			Amount: AmountFromBaseUnits(100000000000, StorxToken),
			JSON:   fmt.Sprintf(`{"value":"1000","currency":"%s"}`, StorxToken.Symbol()),
		},
		{
...
func TestAmountJSONUnmarshal(t *testing.T) {
	tests := []struct {
		...
		{
			JSON:      fmt.Sprintf(`{"value":"100","currency":"%s"}`, StorxToken.Symbol()),
